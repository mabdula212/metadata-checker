...
export class CimbNiagaTransactionParser implements BankTransactionParser {
  readonly bankCode = "CIMB";
  readonly bankName = "CIMB Niaga";

  canParse(input: TransactionParserInput): boolean {
    if (input.bankCode === "CIMB") return true;
    const upper = input.fullText.toUpperCase();
    return upper.includes("CIMB NIAGA") || upper.includes("CIMB");
  }
      ...
      for (const cand of candidates) {
        ...
        if (threeAmounts) {
          ...
          transactions.push({
            ...
            confidence: "HIGH",
            confidenceReasons: ["CIMB 3-column match"],
            pageNumber: page.pageNumber,
          ...
          transactions.push({
            ...
            transactionType: deriveTransactionType(description, isCredit, isDebit),
            confidence: "HIGH",
            confidenceReasons: ["CIMB 2-column match with indicator"],
            pageNumber: page.pageNumber,
            rawText: cand.allText,
          });
        } else {

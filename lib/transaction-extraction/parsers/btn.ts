...
export class BtnTransactionParser implements BankTransactionParser {
  readonly bankCode = "BTN";
  readonly bankName = "Bank Tabungan Negara";

  canParse(input: TransactionParserInput): boolean {
    if (input.bankCode === "BTN") return true;
    const upper = input.fullText.toUpperCase();
    return upper.includes("BANK TABUNGAN NEGARA") || upper.includes("BTN");
  }
      ...
      for (const cand of candidates) {
        ...
        if (threeAmounts) {
          ...
          transactions.push({
            ...
            confidence: "HIGH",
            confidenceReasons: ["BTN 3-column match"],
            pageNumber: page.pageNumber,
          ...
          transactions.push({
            ...
            transactionType: deriveTransactionType(description, isCredit, isDebit),
            confidence: "HIGH",
            confidenceReasons: ["BTN 2-column match with indicator"],
            pageNumber: page.pageNumber,
            rawText: cand.allText,
          });
        } else {

...
export class PermataTransactionParser implements BankTransactionParser {
  readonly bankCode = "PERMATA";
  readonly bankName = "Bank Permata";

  canParse(input: TransactionParserInput): boolean {
    if (input.bankCode === "PERMATA") return true;
    const upper = input.fullText.toUpperCase();
    return upper.includes("PERMATA") || upper.includes("BANK PERMATA");
  }
      ...
      for (const cand of candidates) {
        ...
        if (threeAmounts) {
          ...
          transactions.push({
            ...
            confidence: "HIGH",
            confidenceReasons: ["Permata 3-column match"],
            pageNumber: page.pageNumber,
          ...
          transactions.push({
            ...
            transactionType: deriveTransactionType(description, isCredit, isDebit),
            confidence: "HIGH",
            confidenceReasons: ["Permata 2-column match with indicator"],
            pageNumber: page.pageNumber,
            rawText: cand.allText,
          });
        } else {

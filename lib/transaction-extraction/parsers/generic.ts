...
export class GenericTransactionParser implements BankTransactionParser {
  ...
  canParse(_input: TransactionParserInput): boolean {
    return true; // Always can attempt as fallback
  }
      ...
      for (const cand of candidates) {
        ...
        if (threeAmounts) {
          ...
          transactions.push({
            ...
            confidence: "MEDIUM",
            confidenceReasons: ["Generic 3-column balance match"],
            pageNumber: page.pageNumber,
          ...
          const isCredit =
            rawFlag === "CR" ||
            rawFlag === "+" ||
            parsedMutasi.indicator === "CR" ||
            /setor|bunga|credit|kredit|masuk/i.test(description);
          ...
          transactions.push({
            ...
            transactionType: deriveTransactionType(description, isCredit, isDebit),
            confidence: "MEDIUM",
            confidenceReasons: ["Generic 2-column match"],
            pageNumber: page.pageNumber,
            rawText: cand.allText,
          });
        } else {

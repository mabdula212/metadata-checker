...
export class OcbcTransactionParser implements BankTransactionParser {
  readonly bankCode = "OCBC";
  readonly bankName = "OCBC Indonesia";

  canParse(input: TransactionParserInput): boolean {
    if (input.bankCode === "OCBC") return true;
    const upper = input.fullText.toUpperCase();
    return upper.includes("OCBC") || upper.includes("NISP");
  }
      ...
      for (const cand of candidates) {
        ...
        if (threeAmounts) {
          ...
          transactions.push({
            ...
            confidence: "HIGH",
            confidenceReasons: ["OCBC 3-column match"],
            pageNumber: page.pageNumber,
          ...
          transactions.push({
            ...
            transactionType: deriveTransactionType(description, isCredit, isDebit),
            confidence: "HIGH",
            confidenceReasons: ["OCBC 2-column match with indicator"],
            pageNumber: page.pageNumber,
            rawText: cand.allText,
          });
        } else {
